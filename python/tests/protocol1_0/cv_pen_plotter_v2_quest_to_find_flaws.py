...
#%%
def compute_inverse_kinematics_for_point(x,y):
    ...
    return q1_ax_motion, q2_ax_motion, q1, q2
...
if max_y > max_x:
    ...
    adapted_x_ratio = adapted_y_ratio*xyratio
if max_x > max_y:
    adapted_x_ratio = round(maxsize_armreach_width-1,1)/max_x
    adapted_y_ratio = adapted_x_ratio*yxratio
if max_x == max_y:
    adapted_x_ratio = round(maxsize_armreach_height-1,1)/max_y
    adapted_y_ratio = round(maxsize_armreach_width-1,1)/max_x
...
for iterator in range(len(data)):
    x = (data[iterator][0]*adapted_x_ratio-max_x*adapted_x_ratio/2)/4
    y = (data[iterator][1]*adapted_y_ratio-max_y*adapted_y_ratio/2)/4+30
    x_vals_transformed.append(x)
    y_vals_transformed.append(y)
    
plt.scatter(x_vals_transformed,y_vals_transformed)
plt.axis('equal')
...
for iterator in range(len(data)):
    ...
    q1_ax_motion, q2_ax_motion, q1, q2 = compute_inverse_kinematics_for_point(x,y)
    ...
    if (q1_ax_motion != q1_ax_motion_old):
        ...
        # Plot the reverse calculated arm 1
        ...
        # Plot the reverse calculated arm 2
        ...
        #plt.scatter(x12,y12,c='red',alpha=0.3)
        drawnpoints_x.append(x12)
        drawnpoints_y.append(y12)
        originalpoints_x.append(x)
        originalpoints_y.append(y)


        plt.scatter(drawnpoints_x,drawnpoints_y,c='red',alpha=0.3)
        plt.scatter(originalpoints_x,originalpoints_y,c='green',alpha=0.3)
        sleep(0.0001)
